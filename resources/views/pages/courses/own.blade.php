            ...
            @foreach ($courses as $key => $course)
                <div class="courses__item">
                    ...
                    <div class="courses__course-author">
                        Author: <a href="{{ route('users.show', ['id' => auth()->id()]) }}">{{ auth()->user()->email }}</a>
                    </div>
                    ...
                    @else
                        <a class="courses__course-edit" onclick="document.getElementById('restore-modal-<?= $course->course_id  ?>').style.display = 'flex'">
...
{{--                        <a href="" class="courses__course-edit"><i class="fa-solid fa-arrow-rotate-right"></i></a>--}}
                ...
                <div class="modal" id="restore-modal-{{ $course->course_id }}">
                    <div class="modal-box">
                        <p class="modal-text modal-text-restore mb20 mr20">You sure to <span>restore</span> course: "{{ $course->title }}" ?</p>

                        <div class="modal-buttons">
                            <form class="table-action-form" action="{{ route('courses.restore', ['id' => $course->course_id]) }}" method="post">
                                @csrf
                                @method('post')
                            ...
                            <button onclick="document.getElementById('restore-modal-<?= $course->course_id ?>').style.display = 'none'" class="table-action-button cancel-button">Cancel</button>
                        </div>
                    </div>
                </div>
