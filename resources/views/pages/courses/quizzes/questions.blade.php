    ...
    <div class="list">
        @forelse($questions as $question)
            <div class="list-item">
                ...
                <a href="{{ route('quiz.options.show', ['quiz' => $quiz, 'question' => $question->getKey()]) }}">edit</a>
            </div>
        @empty
...
<script>
    ...
    function saveChanges() {
        ...
        fetch("{{ route('quiz.questions.store', ['quiz' => $quiz]) }}", {
            method: 'POST',
            headers: {
                'Content-Type': 'application/json',
                'Accept': 'application/json',
                'url': "{{ route('quiz.questions.store', ['quiz' => $quiz]) }}",
                "X-CSRF-Token": document.head.querySelector("[name=csrf-token]").content
            },
            body: JSON.stringify(obj),
        }).then(response => {
            if (response.status === 302) {
                window.location.replace("{{ route('quiz.questions.show', ['quiz' => $quiz]) }}");
            }
        });
    }
    ...
    items.forEach(item => {
        item.addEventListener('dblclick', e => {
                ...
                fetch("{{ route('quiz.questions.delete', ['quiz' => $quiz]) }}", {
                    method: 'DELETE',
                    headers: {
                        'Content-Type': 'application/json',
                        'Accept': 'application/json',
                        'url': "{{ route('quiz.questions.delete', ['quiz' => $quiz]) }}",
                        "X-CSRF-Token": document.head.querySelector("[name=csrf-token]").content
                    },
                    body: JSON.stringify(obj),
                }).then(response => {
